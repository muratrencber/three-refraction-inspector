...
const bvh = new BVH();
const bvhPromise = bvh.loadModelAndConstruct(new FBXLoader().loadAsync("./models/cat.fbx"));
Promise.all([envMapPromise, bvhPromise]).then(([envMap, bvhResult]) => {
    const {model, bvh} = bvhResult;
    //let model = xubeMesh;
    model.scale.set(0.3, 0.3, 0.3);
    model.translateY(-2);
    model.updateMatrixWorld();
    const bvhMat = new MeshRefractiveBVHMaterial(bvh, {
        ior: 2.41,
...
const voxelPromise = new SparseVoxelOctree().loadModelAndConstruct(new FBXLoader().loadAsync("./models/Diamond.fbx"), 2, ContouringMethod.AverageNormals);
Promise.all([envMapPromise, voxelPromise]).then(([envMap, voxelResult]) => {
    return;
    const {model, svo} = voxelResult;
    model.updateMatrixWorld();
