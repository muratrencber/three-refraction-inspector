...
const bvhPromise = new SparseVoxelOctree(false).loadModelAndConstruct(new FBXLoader().loadAsync("./models/Diamond.fbx"), 5);//bvh.construct(xubeMesh);
Promise.all([envMapPromise, bvhPromise]).then(([envMap, bvhResult]) => {
    const {model, svo} = bvhResult;
    //let model = xubeMesh;
    model.updateMatrixWorld();
    const bvhMat = new MeshRefractiveSVOMaterial(svo, {
        ior: 2.41,
